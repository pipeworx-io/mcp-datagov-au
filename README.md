...
`tools/list` at `https://gateway.pipeworx.io/datagov-au/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/datagov_au_search \
  -H 'Content-Type: application/json' \
  -d '{"query":"bushfire"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/datagov_au_search`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "datagov-au": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-datagov-au"]
    }
  }
}
```
